...
#define LED_ON()  do {stm32f4_gpio_set_output(STM32F4_GPIO_PIN(0,6), 1); \
			stm32f4_gpio_set_output(STM32F4_GPIO_PIN(0,7), 0); \
		}	while(0)
#define LED_OFF() do { stm32f4_gpio_set_output(STM32F4_GPIO_PIN(0,6), 0) ;\
			stm32f4_gpio_set_output(STM32F4_GPIO_PIN(0,7), 1); \
	}while(0)
